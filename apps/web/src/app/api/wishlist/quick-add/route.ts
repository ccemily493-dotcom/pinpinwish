...
import { createManualWishlistItem } from '@/lib/db/repository'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url && !fallbackImageUrl && !fallbackTitle) {
      return apiError('Debes proporcionar una URL de producto, imagen o nombre.', 400, 'INVALID_INPUT')
    }
    ...
    if (url && isSafeExternalUrl(url)) {
      try {
        ...
        console.warn(`[quick-add] Error parsing external URL ${url}:`, fetchErr)
      }
    }

    const finalName = parsedName || 'Nuevo Producto'

    // Try downloading image locally if available
    let localImagePath: string | undefined
    if (parsedImageUrl && parsedImageUrl.startsWith('http')) {
      try {
        const downloadRes = await downloadPinImage(parsedImageUrl, `drop_${Date.now()}`, { timeoutMs: 8000 })
        if (downloadRes) {
    ...
    const item = createManualWishlistItem({
      ...
      resolution: assessDroppedProductResolution({
        sourceType: url ? 'manual_url' : 'image',
